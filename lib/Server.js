...
export class Server {
  ...
  async apply(namespaceId) {
    let params = {};
    if (namespaceId) {
      params.Namespaces = String(namespaceId);
    }
   ...
   * provided.
   * @param {number | undefined} namespaceId ID of the namespace to deploy
   */
  async deploy(namespaceId) {
    let params = {};
    if (namespaceId) {
   ...
   *
   * @param {number | undefined} namespaceId ID of the namespace to generate transactions in
   */
  async generate(namespaceId) {
    let params = {};
    if (namespaceId) {
      params.Namespace_ID = namespaceId;
    }
  ...
  async getNamespace(namespace) {
    this.logServerMessage("Getting namespace information...");
