...
import { Client, FileUploader } from "@olenbetong/data-object/node";
...
export class Server {
  ...
  async addArticlePermission({ hostname, articleId, roleId }) {
  ...
  async download(namespace) {
    ...
    while (retry) {
      try {
        ...
        this.logServerMessage(
          `Failed to download transactions from ${this.hostname} (${error.message}). Retrying in 1 second...`
        );
        attempt++;
   ...
   */
  async createBundle(name, namespaceId) {
   ...
   */
  async publishWebAsset(hostname, name, type, description) {
    this.logServerMessage(
   ...
   */
  async uploadTemplate(hostname, templateId, content, contentTest) {
   ...
   */
  async uploadSiteScript(hostname, scriptId, content, contentTest) {
   ...
   */
  async uploadSiteStyle(hostname, styleId, content, contentTest) {
   ...
   */
  async copyDatasourcesFromDev(hostname, articleId) {
    this.logServerMessage(
