...
import { FileUploader } from "@olenbetong/data-object";
...
export class Server {
   ...
   */
  async assertOnlyOneTransaction(filter, name, namespace) {
   ...
   * @param {string} version Version comments to set as description
   */
  async publishTemplate(hostname, templateId, version) {
    this.logServerMessage(`Getting template '${hostname}/${templateId}'...`);
    this.dsTemplates.setParameter(
      "whereClause",
      `[HostName] = '${hostname}' AND [TemplateID] = '${hostname}/${templateId}'`
    );
    await this.dsTemplates.refreshDataSource();

    if (this.dsTemplates.getDataLength() > 0) {
      this.logServerMessage(
        `Publishing template '${hostname}/${templateId}'...`
      );
      await this.procPublishWebAsset.execute({
        Hostname: this.dsTemplates.currentRow("HostName"),
        Name: this.dsTemplates.currentRow("TemplateID"),
        Type: "Template",
        Description: version,
        IssueID: null,
      });
      ...
      this.logServerMessage(
        chalk.yellow(`Template '${hostname}/${templateId}' not found.`)
      );
    }
  }
   ...
   * @param {string} version Version comments to set as description
   */
  async publishSiteScript(hostname, scriptId, version) {
    this.logServerMessage(`Getting site script '${hostname}/${scriptId}'...`);
    this.dsSiteScripts.setParameter(
      "whereClause",
      `[HostName] = '${hostname}' AND [Name] = '${hostname}/${scriptId}'`
    );
    await this.dsSiteScripts.refreshDataSource();

    if (this.dsSiteScripts.getDataLength() > 0) {
      this.logServerMessage(`Publishing script '${hostname}/${scriptId}'...`);
      await this.procPublishWebAsset.execute({
        Hostname: this.dsSiteScripts.currentRow("HostName"),
        Name: this.dsSiteScripts.currentRow("Name"),
        Type: "Script",
        Description: version,
        IssueID: null,
      });
      ...
      this.logServerMessage(
        chalk.yellow(`Script '${hostname}/${scriptId}' not found.`)
      );
    }
  }
   ...
   * @param {string} version Version comments to set as description
   */
  async publishSiteStyle(hostname, styleId, version) {
    this.logServerMessage(
      `Getting site stylesheet '${hostname}/${styleId}'...`
    );
    this.dsSiteStyles.setParameter(
      "whereClause",
      `[HostName] = '${hostname}' AND [Name] = '${hostname}/${styleId}'`
    );
    await this.dsSiteStyles.refreshDataSource();

    if (this.dsSiteStyles.getDataLength() > 0) {
      this.logServerMessage(
        `Publishing stylesheet '${hostname}/${styleId}'...`
      );
      await this.procPublishWebAsset.execute({
        Hostname: this.dsSiteStyles.currentRow("HostName"),
        Name: this.dsSiteStyles.currentRow("Name"),
        Type: "Style",
        Description: version,
        IssueID: null,
      });
      ...
      this.logServerMessage(
        chalk.yellow(`Stylesheet '${hostname}/${styleId}' not found.`)
      );
    }
  }
   ...
   * @param {"test" | "prod"} mode Whether to upload to test or production mode
   * @param {string} content Content to upload
   */
  async uploadTemplate(hostname, templateId, mode, content) {
    this.logServerMessage(`Getting template '${hostname}/${templateId}'...`);
    this.dsTemplates.setParameter(
      "whereClause",
      `[HostName] = '${hostname}' AND [TemplateID] = '${hostname}/${templateId}'`
    ...
    if (this.dsTemplates.getDataLength() === 0) {
      ...
      this.dsTemplates.currentRow("TemplateID", templateId);
    }

    this.dsTemplates.currentRow(
      mode === "test" ? "HTMLTemplateTest" : "HTMLTemplate",
      content
    );
   ...
   * @param {string} content Content to upload
   */
  async uploadSiteScript(hostname, scriptId, mode, content) {
    this.logServerMessage(`Getting site script '${hostname}/${scriptId}'...`);
    this.dsSiteScripts.setParameter(
      "whereClause",
      `[HostName] = '${hostname}' AND [Name] = '${hostname}/${scriptId}'`
    ...
    this.dsSiteScripts.currentRow(
      mode === "test" ? "ScriptContentTest" : "ScriptContent",
      content
    );
   ...
   * @param {string} content Content to upload
   */
  async uploadSiteStyle(hostname, styleId, mode, content) {
    ...
    this.dsSiteStyles.currentRow(
      mode === "test" ? "StyleContentTest" : "StyleContent",
      content
    );
