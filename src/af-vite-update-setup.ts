...
import { access, copyFile, mkdir, readdir, writeFile, unlink, rename } from "node:fs/promises";
...
async function removePackageIfExists(packages: string[], dependencies: string[]) {
...
async function upgradePackageConfig(pkg: any) {
	...
	pkg.scripts.start = isVite ? "af vite generate-types && vite dev" : "af vite generate-types && react-scripts start";
	...
	pkg.browserslist = {
		production: ["last 6 chrome versions", "last 6 firefox versions", "safari >= 12"],
...
async function updateProjectSetup() {
	...
	let dependencies = [...Object.keys(pkg.dependencies ?? {}), ...Object.keys(pkg.devDependencies ?? {})];
