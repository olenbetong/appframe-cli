...
async function getResourceDefinition(
  ...
  if (options.overrides) command += ` --overrides "${options.overrides}"`;
  if (options.distinct) command += " --distinct";
  if (options.aggregates) command += ` --aggregates ${options.aggregates}`;
  if (options.groupBy) command += ` --group-by ${options.groupBy}`;
  if (options.where) command += ` --where "${options.where}"`;
