...
program
	...
	.description("Commands to use when working with a SynergiWeb application using Vite")
	.configureHelp({
	...
	.alias("gt")
	.command("update-setup", "Ensures the project has deploy/publish scripts set up")
	.command("update", "Updates individual project files")
	.command("publish", "Publish the article, and take the transaction from dev to prod (download only)")
	.command("init <name>", "Bootstraps a new application using Vite with @olenbetong/appframe-vite")
	...
	.command("copy-datasources", "Copies datasources for an article from SynergiDev to SynergiStage")
	.alias("cd")
	.command(
		"release",
