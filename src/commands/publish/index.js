...
const types = ["article-script", "article-style", "component-global", "component-site", "site-script", "site-style"];
...
function getConfigFromArgs(args) {
  const config = {};
  const potentialArgs = ["article", "domain", "exclude", "hostname", "mode", "password", "source", "target", "user"];
  ...
  try {
    ...
    if (type === "article-script" || type === "article-style") {
      ...
      if (idx > 0) {
        ...
        throw new Error(
          'To publish article scripts or styles with array shorthand, use "articleName/target" as the target.'
        );
      }
    }
...
function getTargetPath(type, name) {
  if (type === "article-script") {
    return `/file/article/script/${name}`;
  } else if (type === "article-style") {
    return `/file/article/style/${name}`;
  } else if (type === "component-global") {
    return `/file/component/${name} (GLOBAL)`;
  } else if (type === "component-site") {
    return `/file/component/${name} (LOCAL)`;
  } else if (type === "site-script") {
    return `/file/site/script/${name}`;
  } else if (type === "site-style") {
    return `/file/site/style/${name}`;
  }
...
async function publishItem(client, item) {
  ...
  console.log(`Publish (${hostname}): '${source}' -> '${getTargetPath(type, target)}'`);
...
async function publish(args) {
  ...
  const config = Object.assign(defaultTargetProperties, configFromFile, getConfigFromArgs(args));
